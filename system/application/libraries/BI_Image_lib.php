 ...
 */
class BI_Image_lib extends CI_Image_lib {
	 ...
	 * @return	void
	 */	
	function BI_Image_lib($props = array())
	{
		if (count($props) > 0)
		{
	 ...
	 * convert_to_jpeg
	 *
	 * converting image to jpeg. it uses for tiff, raw formats only
	 *
	 * @author  Popov
	 * @class   BI_Image_lib
	 * @access  public
	 * @param   string     $file_path_in  
	 * @param   string     $file_path_out  
	 * @return  bool  
	 */
	function convert_to_jpeg($file_path_in, $file_path_out) {
		try {
			$CI =& get_instance();
			$CI->config->load('../modules/gallery_mod/config/config');
			$path_to_convert = $CI->config->item('path_to_convert');
			log_message('debug', 'convert: '.$path_to_convert." ".$file_path_in." ".$file_path_out);
			exec($path_to_convert." ".$file_path_in." ".$file_path_out, $errors);
			
			if(empty($errors)) 
				return true;
			else 
				log_message('error', 'convert_to_jpeg.'.var_export($errors));			
