...
// 		 * It can either set to GMT or time(). The pref
    ...
    */
    ...
    function remember_me($login, $password, $lifetime)
    {    	
        $this->del_auth_cookies();   
    	$this->set_auth_cookies($login, $password, $lifetime);  	
    }
    
    function del_auth_cookies()
    {
    	$this->object->load->helper('cookie');
    	if (isset($_COOKIE[$this->sess_cookie . '_login'])) 		
               delete_cookie($this->sess_cookie . '_login');        
        if (isset($_COOKIE[$this->sess_cookie . '_pwd'])) 		
               delete_cookie($this->sess_cookie . '_pwd');
    }
    
    function set_auth_cookies($login, $password, $lifetime)
    {
    	setcookie(
					$this->sess_cookie . '_login', 
					$login,
					$this->now + $lifetime, 
					$this->object->config->item('cookie_path'), 
					$this->object->config->item('cookie_domain'), 
    ...
    function check_auth_cookies()
    {
      	if (isset($_COOKIE[$this->sess_cookie . '_login'])) 
      	{
      		$login = $this->object->input->cookie($this->sess_cookie. '_login');
			if ($login === FALSE)
				return FALSE;
     	}
     	else
     	...
     	if (isset($_COOKIE[$this->sess_cookie . '_pwd'])) 
      	{
      		$pwd = $this->object->input->cookie($this->sess_cookie. '_pwd');
			if ($pwd === FALSE)
				return FALSE;
     	}
     	else
      	...
      	// Is such user in the DB?
		$this->object->db->where('login', $login);
		$this->object->db->where('password', $pwd);
		
		$query = $this->object->db->get('users');
		...
		{
			 if (modules::run('users_mod/users_ctr/authorize_from_cookie', $login, $pwd))
			 	return TRUE;
			 else
			 	return FALSE;				
		}
