...
function kh_acl_check($aro, $aco, $axo = null)
{
	...
	*/
	$rs = $ci->db->query('SELECT access.allow
                            FROM '.$ci->db->dbprefix.$tables['aros'].' AS aro_node INNER JOIN '.$ci->db->dbprefix.$tables['acos'].' AS aco_node
                              LEFT JOIN '.$ci->db->dbprefix.$tables['aros'].' AS aro_branch ON (aro_node.lft >= aro_branch.lft AND aro_node.lft <= aro_branch.rgt)
                              LEFT JOIN '.$ci->db->dbprefix.$tables['acos'].' AS aco_branch ON (aco_node.lft >= aco_branch.lft AND aco_node.lft <= aco_branch.rgt)
                              INNER JOIN '.$ci->db->dbprefix.$tables['access'].' AS access ON (aro_branch.id = access.aro_id AND aco_branch.id = access.aco_id)
                            WHERE aro_node.name = ? AND aco_node.name = ?
                            ...
                            LIMIT 1', array($aro, $aco));
    /*
SELECT access.allow FROM khacl_access AS access, khacl_aros AS aro_node  
LEFT JOIN khacl_aros AS aro_branch ON (aro_node.lft >= aro_branch.lft AND aro_node.lft <= aro_branch.rgt)
, khacl_acos AS aco_node 
LEFT JOIN khacl_acos AS aco_branch ON (aco_node.lft >= aco_branch.lft AND aco_node.lft <= aco_branch.rgt) 
 
WHERE (aro_branch.id = access.aro_id AND aco_branch.id = access.aco_id) and aro_node.name = 'alina' AND aco_node.name = 'Album' 
ORDER BY aro_branch.rgt ASC, aco_branch.rgt ASC LIMIT 1
* */

    /*
SELECT access.allow 
FROM khacl_aros AS aro_node
inner join khacl_acos AS aco_node  on aco_node.name = 'album' 
LEFT JOIN khacl_aros AS aro_branch ON (aro_node.lft >= aro_branch.lft AND aro_node.lft <= aro_branch.rgt) 
LEFT JOIN khacl_acos AS aco_branch ON (aco_node.lft >= aco_branch.lft AND aco_node.lft <= aco_branch.rgt) 
INNER JOIN khacl_access AS access ON (aro_branch.id = access.aro_id AND aco_branch.id = access.aco_id) 
WHERE aro_node.name = 'alina' 
    ...
    if ($rs->num_rows() == 1)
    {
        ...
        if ($row->allow == 'Y')
        {
            if ($axo !== null)
            {
                ...
                */
				$rs = $ci->db->query('SELECT access_actions.allow
                                        FROM '.$ci->db->dbprefix.$tables['aros'].' AS aro_node INNER JOIN '.$ci->db->dbprefix.$tables['acos'].' AS aco_node INNER JOIN '.$ci->db->dbprefix.$tables['axos'].' AS axo_node
                                          LEFT JOIN '.$ci->db->dbprefix.$tables['aros'].' AS aro_branch ON (aro_node.lft >= aro_branch.lft AND aro_node.lft <= aro_branch.rgt)
                                          LEFT JOIN '.$ci->db->dbprefix.$tables['acos'].' AS aco_branch ON (aco_node.lft >= aco_branch.lft AND aco_node.lft <= aco_branch.rgt)
                                          LEFT JOIN '.$ci->db->dbprefix.$tables['access'].' AS access ON (aro_branch.id = access.aro_id AND aco_branch.id = access.aco_id)
                                          INNER JOIN '.$ci->db->dbprefix.$tables['access_actions'].' AS access_actions ON (access.id = access_actions.access_id AND axo_node.id = access_actions.axo_id)
